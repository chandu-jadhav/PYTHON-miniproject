...
def Ok():
    ...
    elif(uname == "Admin" and password == "bvcoe"):
...
def show():
    ...
    if (var1.get()):
        ...
        qty = int(e6.get())
        tot = int(price * qty)
        tempList = [['Hand Sanitizer', e1.get(), e6.get(), tot]]
    ...
    if (var2.get()):
        ...
        qty = int(e7.get())
        tot = int(price * qty)
        tempList = [['Hand Glowes', e2.get(), e7.get(), tot]]
        tempList.sort(key=lambda e: e[1], reverse=True)
        for i, (item, price, qty, tot) in enumerate(tempList, start=1):
            listBox.insert("", "end", values=(item, price, qty, tot))
    ...
    if (var3.get()):
        ...
        qty = int(e8.get())
        tot = int(price * qty)
        tempList = [['Face Mask', e3.get(), e8.get(), tot]]
        tempList.sort(key=lambda e: e[1], reverse=True)
        for i, (item, price, qty, tot) in enumerate(tempList, start=1):
            listBox.insert("", "end", values=(item, price, qty, tot))
    ...
    if (var4.get()):
        ...
        qty = int(e9.get())
        tot = int(price * qty)
        tempList = [['Antiseptic Liquid', e4.get(), e9.get(), tot]]
        tempList.sort(key=lambda e: e[1], reverse=True)
        for i, (item, price, qty, tot) in enumerate(tempList, start=1):
            listBox.insert("", "end", values=(item, price, qty, tot))
...
var1 = IntVar()
Checkbutton(root, text="Hand Sasanitizer", variable=var1).place(x=10, y=50)
 
var2 = IntVar()
Checkbutton(root, text="Face Mask", variable=var2).place(x=10, y=80)
 
var3 = IntVar()
Checkbutton(root, text="Hand Glowes", variable=var3).place(x=10, y=110)
 
var4 = IntVar()
Checkbutton(root, text="Antiseptic", variable=var4).place(x=10, y=140)
